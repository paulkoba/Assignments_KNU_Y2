...
class Solution {
private:
    bool backtrack(vector<int>& color, unordered_map<int, vector<int>>& paths, int N, int k) {
        if(k == N) return true;
            
        //Pre-select which types N-th garden can possibly be of.
        vector<int> temp = {0, 1, 1, 1, 1}; //First value is dummy.
...
public:
    vector<int> gardenNoAdj(int N, vector<vector<int>>& paths) {
        ...
        unordered_map<int, vector<int>> paths_transformed;
        for(auto& vec : paths) {
