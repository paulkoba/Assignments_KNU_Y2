...
public:
    bool isInterleave(string s1, string s2, string s3, size_t i1 = 0, size_t i2 = 0, size_t i3 = 0) {
        ...
        for(size_t i = i3; i < s3.size(); ++i) {
            ...
            bool res = isInterleave(s1, s2, s3, idx1 + 1, idx2, i + 1);
            if(res) return true;
            
            result[make_pair(i1, i2)] = result[make_pair(idx1 + 1, idx2)] = false;
            
            return result[make_pair(i1, i2)] = result[make_pair(idx1, idx2 + 1)] = isInterleave(s1, s2, s3, idx1, idx2 + 1, i + 1);
        }
